...
-   Checked for multicollinearity using the Variance Inflation Factor
    (VIF)
-   Obtained VIF results using the car::vif function
...
assessing linearity between time in hospital and logit of readmission
...
Assessing linearity between log of time in hospital and logit of
readmission. relationship appear more linear.
...
Assessing linearity between number of lab procedures and logit of
readmission
...
Variables *number_outpatient, number_inpatient and number_emergency* are
too imbalanced to assess linearity because 1st - 3rd quartiles are all
0. Only one point shows on the plot. It is not possible to assess the
linearity to the logit for these variables in this way. Test on
*preceding_visits* instead.

```{r}

var<- dfC$preceding_visits

dfC$readmitted_2<- as.factor(dfC$readmitted_2)
summary(dfC$preceding_visits)

# check first quartile- 2 and days and less
table(dfC$readmitted_2[var<0])

p1 <- mean(var <0)
p2 <- mean(var >=0 & var <0)

p3 <- mean(var >= 0 & var <0.2312)
...
plot(meds, logits, main= "should be linear if tih is linear related to log odds of readmitted_2", ylab= "log odds")
...
Borderline-SMOTE (BL SMOTE)

Description:

Generate synthetic positive instances using Borderline-SMOTE algorithm.
The number of majority neighbor of each minority instance is used to
divide minority instances into 3 groups; SAFE/DANGER/NOISE, only the
DANGER are used to generate synthetic instances.
...
Generate synthetic positive instances using ADASYN algorithm. The number
of majority neighbors of each minority instance determines the number of
synthetic instances generated from the minority instance.
...
## Unbalanced data, full set of predictors
...
## Unbalanced data, significant variables
...
## Balanced data, full set of predictors
...
-   Extract predictors and outcome variables from the training dataset.

-   Prepare the test dataset by removing the outcome variable and its
    binary representation.

-   Fit a LASSO logistic regression model using cross-validated glmnet,
    specifying the binomial family and alpha as 1.

-   Plot the cross-validated mean squared error (CV MSE) as a function
    of lambda.

-   Extract the optimal lambda value from the model.

-   Print coefficients of the selected variables at the optimal lambda
    value.

-   Use the model to predict outcomes for the test dataset.

-   Convert predicted probabilities to binary class predictions (0 or 1)
    using a threshold of 0.5.

-   Create a confusion matrix to evaluate the model's performance on the
    test dataset.

-   Plot the ROC curve to assess the model's performance further

-   Calculate the AUC (Area Under the Curve) value and print it.
...
```
The stepwise selection procedure using both forward and backward steps was employed to identify the most significant predictors of readmission likelihood. The forward stepwise selection procedure started with an initial model including various demographic and clinical features. Notably, after evaluating the model fit using the AIC, the stepwise procedure identified log_scores, num_medications, and gender as non-significant predictors and excluded them from the final model. 

Conversely, the backward stepwise selection procedure, starting with the same initial model, identified log_scores and num_medications as non-significant predictors and removed them from the final model. Both forward and backward stepwise procedures converged to similar final models, comprising significant predictors such as age, race, specialty, discharge status, admission source, admission type, A1Cresult, change in medication, log-transformed hospitalization time, transformed number of lab procedures, insulin usage, the number of medications, preceding visits, and diabetes medication usage. 
...
-   Checked for multicollinearity using the Variance Inflation Factor on the model with balanced data and significant predictors 
    (VIF)
-   Obtained VIF results using the car::vif function
-   No significant multicollinearity was detected
...
- A vector named `coef` is defined to store the estimated coefficients from the logistic regression model. Each coefficient corresponds to a predictor variable in the model.

-  The `exp()` function is utilized to exponentiate each coefficient, resulting in the calculation of the odds ratio for each predictor variable. This transformation converts the coefficients from the log-odds scale to the odds ratio scale.

-  A `for` loop is employed to iterate through each variable in the `coef` vector. During each iteration, the variable name and its corresponding odds ratio are printed. This facilitates a clear display of the odds ratios for each predictor variable in the model.
...
## LR, balanced, significant, standardized
...
-   Trained model applied to test set, generating predictions
    (testsetpreds_rf).
-   Confusion matrix computed to evaluate model performance.
-   Performance metrics (sensitivity, specificity, accuracy) calculated.
-   ROC curve plotted, indicating model discrimination ability.
-   AUC value printed as summary of ROC curve, representing overall
    model performance.
