...
NOTE: Run the "1 data prep" file first
...
#custom_colors <- c("#5236ab", "#009E73", "#F55E00", "#bfb5f9", "#e6e3f3", "#E69F00", "#F5B4E9", "#A5B4E9","#15BE37","#241571", "#FFC300", "#DAF7A6", "#9e83f5", "#e6e3f3")
...
# Plot to visualize the custom color palette
plot(1:length(custom_colors), type = "n", axes = FALSE, xlab = "", ylab = "", main = "Custom Color Palette", ylim = c(0, 1), xlim = c(0, length(custom_colors)))
for (i in 1:length(custom_colors)) {
  rect(i - 1, 0, i, 1, col = custom_colors[i], border = NA)
}
...
numeric_columns <- c( "time_in_hospital", "num_lab_procedures", "num_medications", "num_procedures", "number_outpatient", "number_inpatient", "number_emergency")
...
ggplot(df, aes(x = readmit, fill = readmit)) +
  geom_bar() +
  labs(title = "Readmission Status",
       x = "Readmitted", y = "Frequency") +
...
ggplot(dfC, aes(x = readmit, fill = readmit)) +
  geom_bar() +
  labs(title = "Patient Readmission Status",
       x = "Readmitted", y = "Frequency") +
  scale_fill_manual(values = custom_colors) +
  guides(fill = FALSE) # Set custom bar colors
...
ggplot(dfC, aes(x = age, fill = age)) +
  geom_bar() +
  labs(title = "Age Profile of Patients",
       x = "age", y = "Frequency") +
  scale_fill_manual(values = custom_colors) +
...
```{r}
# Create a   the race variable with custom colors
ggplot(dfC, aes(x = race, fill = race)) +
  geom_bar() +
  labs(title = "Race Profile of Patients",
       x = "Race", y = "Frequency") +
  scale_fill_manual(values = custom_colors) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  guides(fill = FALSE)
...
ggplot(dfC, aes(x = gender, fill = gender)) +
  geom_bar() +
  labs(title = "Gender Profile of Patients",
...
ggplot(dfC, aes(x = fct_infreq(admission_source_id), fill = admission_source_id)) +
  geom_bar() +
  labs(title = "  Admission Source ID",
...
```{r}
# Create a   the specialty variable
...
ggplot(dfC, aes(x = change, fill = change)) +
  geom_bar() +
  labs(title = "Change in Medication",
...
ggplot(dfC, aes(x = diabetesMed, fill = diabetesMed)) +
  geom_bar() +
  labs(title = "Diabetes Medication",
...
ggplot(dfC, aes(x = medication_type, fill = medication_type)) +
  geom_bar() +
  labs(title = "Patient medication type",
       x = "Medication type", y = "Frequency") +
  scale_fill_manual(values = custom_colors) +
  guides(fill = FALSE)
...
ggplot(dfC, aes(x = log_time_hosp)) +
  geom_histogram(fill = custom_colors[5], color = "black", bins = 10, binwidth=0.25) +
...
ggplot(dfC, aes(x = num_lab_procedures)) +
  geom_histogram(fill = custom_colors[3], color = "black", bins = 132) +
  labs(title = "Histogram of Number of Lab Procedures",
       x = "Number of Lab Procedures", y = "Frequency") +
  custom_theme
...
ggplot(dfC, aes(x = standardized_num_lab)) +
  geom_histogram(fill = custom_colors[3], color = "black", bins = 132, binwidth=0.25) +
...
ggplot(dfC, aes(x = number_inpatient)) +
...
ggplot(dfC, aes(x = number_emergency)) +
  geom_histogram(fill = custom_colors[3], color = "black", bins = 30) +
...
wilcox_test_result <- wilcox.test(num_lab_procedures ~ readmitted, data = dfC)
...
ggplot(dfC, aes(x = num_lab_procedures, fill = factor(readmitted))) +
  ...
  scale_fill_manual(values = c("#9e83f5", "#e6e3f3"))+
...
# Perform Wilcoxon rank-sum test
wilcox_test_result <- wilcox.test(num_medications ~ readmitted, data = dfC)
...
# Perform Wilcoxon rank-sum test
wilcox_test_result <- wilcox.test(num_procedures ~ readmitted, data = dfC)
...
ggplot(dfC, aes(x = A1Cresult, fill = readmitted_2)) +
  geom_bar(position = "dodge", color = "black") +
  labs(title = "  A1Cresult and Readmitted",
       x = "A1Cresult", y = "Proportion",
       fill = "Readmitted") +
  scale_fill_manual(values = custom_colors[2:5]) +  # Use custom colors for fill
  custom_theme
...
```{r}
ggplot(data = dfC, aes(x = number_diagnoses, fill = factor(readmitted))) + 
  geom_bar(position = "fill") + 
...
# Perform Wilcoxon rank-sum test
wilcox_test_result <- wilcox.test(number_diagnoses ~ readmitted, data = dfC)
...
ggplot(dfC, aes(x = diag_1, fill = readmitted_2)) +
  ...
  labs(title = "  diag_1 and Readmitted",
...
ggplot(dfC, aes(x = medication_type, fill = factor(readmitted))) +
  geom_bar(position = "fill") +
  labs(title = "  Medication Type against Readmission",
       x = "Medication Type", y = "Proportion of Readmitted") +
  scale_fill_manual(values = c("#5236ab", "#cbc3e6")) 
...
ggplot(dfC, aes(x = discharge, fill = factor(readmitted))) +
  geom_bar(position = "fill") +
  labs(title = "  Discharge against Readmission",
...
ggplot(dfC, aes(x = admission_type_id, fill = factor(readmitted))) +
  geom_bar(position = "fill") +
  labs(title = "Admission Type and Readmission",
...
ggplot(dfC, aes(x = race, fill = factor(readmitted))) +
  geom_bar(position = "fill") +
  labs(title = "Race and Readmission",
...
ggplot(dfC, aes(x = gender, fill = factor(readmitted))) +
  geom_bar(position = "fill") +
  labs(title = "Gender and Readmission",
...
ggplot(dfC, aes(x = scores, fill = factor(readmitted_2))) +
  ...
  labs(title = "Histogram of Charlson Comorbidity Scores by Readmission",
       ...
       fill = "Readmitted") +
  scale_fill_manual(values = c("#5236ab", "#e6e3f3")) +  # Custom colors for readmission status
...
ggplot(dfC, aes(x = A1Cresult, fill = factor(readmitted))) +
  ...
  labs(title = "A1Cresult against Readmission",
